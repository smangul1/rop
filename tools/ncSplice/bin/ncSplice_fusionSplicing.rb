...
require_relative "../lib/fusionSplicing.rb"
...
begin	
	Analysis.prepare_anchorpairs(unmapped_reads, anchor_length, "#{prefix}_anchors.fastq")
 	...
 	anchor_pairs = Open3.popen3("samtools view #{prefix}.bam") do |stdin, stdout, stderr, t|
 		FusionSplice.process_bam(stdout, chromosome_files, skip)
 	end

 	FusionSplice.seed_extension(anchor_pairs, anchor_length, read_length, chromosome_files, "#{prefix}_candidateReads.txt")	
 	FusionSplice.collaps_qnames("#{prefix}_candidateReads.txt", "#{prefix}_candidates.txt")
 	FusionSplice.candidates2fa("#{prefix}_candidates.txt", chromosome_files, read_length, "#{prefix}_faIndex.fa")
 	...
 	Open3.popen3("samtools view #{prefix}_remapping.bam") do |stdin, stdout, stderr, t|
 		FusionSplice.remapped_reads(stdout, "#{prefix}_remapped.txt", read_length)
 	end
 	
 	FusionSplice.final_candidates("#{prefix}_candidates.txt", "#{prefix}_remapped.txt", "#{prefix}_final.txt")
	
rescue StandardError => err
	$logfile.puts "#{Time.new}: Error in ncSplice_fusionSplice.rb"
